...
Updated September 16, 2026 after the verified and replayed [pediatric liver panel](reports/psc-liver-programs.md). The separate frozen BACH2 endpoint has also passed numerical audit and scientific replay; its aggregate reporting remains in progress.

**Current priority: complete reporting and milestone publication, not new computation.** The fixed liver panel has zero nominal, BH or BY discoveries among 30 Core hypotheses. The single BACH2 RNA check is already computed, independently verified and replayed. Both were prospectively frozen and neither outcome authorizes different endpoints, thresholds, cohorts or a model sweep. The external epithelial IL17 transfer remains HOLD.
...
| **Completed; results milestone** | Pediatric liver disease-control programs | [Full results](reports/psc-liver-programs.md): all 30 Core P ≥ 0.382808; 0 BH/BY. Focal difference −0.0211, 95% CI [−0.1547, +0.1125]. Complete source/native audit and all 12 native artifacts replay exactly. | 17 PSC / 17 ASC / 30 AIH; whole biopsy, no healthy group or qualified clinical/stage adjustment. Not equivalence or absence of cell-specific biology. All 30 Strict rows remain sensitivity only; IL17 transfer HOLD. |
...
| **Computed, verified and replayed; reporting pending** | BACH2 published naïve-CD4 RNA observation | Pushed pre-effect freeze `a1b0a6e`; one [fixed endpoint](docs/psc-bach2-naive-rna-protocol.md), complete independent eight-matrix audit and exact scientific replay. Publish every aggregate outcome and fixed diagnostic next. | Four/four male PSC donors, observed C0/C1 mixture/RNA-content weighting, clinical/LD/batch limits. Not protein/translation, causal allele, cell-level replication or independent validation. Individual quantities remain private. |
...
The [initial finite frontier review](reports/psc-analysis-frontier-review.md) set contrasts, limits and stopping rules. The [final cached reserve appraisal](reports/psc-remaining-frontier-appraisal.md) parked GSE255234 and GSE303012 for limited incremental discrimination—not invented access barriers. Both admitted numerical questions are now complete, independently verified and replayed. Finish full aggregate reporting, preservation/privacy checks and milestone pushes, then close this computational phase. Neither result opens another source or model sweep. This practical boundary does not claim that all possible PSC science is exhausted.
