...
| **Frozen; execution next** | Does a fixed ETS2 program add stimulus information beyond broad inflammation/stress programs? | Independent healthy-line stimulus context and held-out line/run behavior. | MacroMap source/design qualification is complete; freeze program mapping, reference matching, run/line holdout and endpoints. No PSC-causality claim. |
| **Unconditional next-stage question; not executed** | Do externally fixed epithelial/injury programs distinguish PSC from other autoimmune/chronic liver injury? | Disease-tissue relevance and a disease-control comparison regardless of the organoid outcome. | GSE303271 count interface qualifies; gene/program mapping is active. GSE159676 is held for mixed-scale values and repeated-biopsy/label issues. Fix programs and clinical limits before tissue effects. An organoid-derived endpoint needs its own explicit discovery/transfer plan. |
