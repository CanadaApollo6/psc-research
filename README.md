...
The latest [ETS2 program benchmark](reports/ets2-program-benchmark.md) measures a published macrophage gene set in PSC liver cells, with expression-matched references and broad inflammation/stress comparisons. **906 of 927 intended genes map; the primary activated-macrophage score is lower than Kupffer cells in all four eligible donor pairs**, including after removing direct comparator-gene overlap. Related disruption experiments agree, but overexpression reverses the program only partially. This is useful evidence about transfer between cell states, not a validated disease-activity score. A separate [inhibitor-method reconstruction](docs/ets2-inhibitor-reconstruction.md) reproduces all nine archived pathway enrichment scores from the released ranks; the original count fit and rank-export provenance remain incomplete.
