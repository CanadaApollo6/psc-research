...
The [PRKD2 signal analysis](reports/prkd2-signal-analysis.md) remains **inconclusive for a shared PSC–monocyte RNA cause**. Complete published RNA component vectors and a full-allele recovery still leave only about 80% of the PSC signal represented; all 72 component/prior/sample-size settings fail the fixed coverage requirement. One missing variant, rs112445263, carries about 20% of PSC evidence and has no association rows in either RNA file. Earlier expression-direction agreement remains intact. Independent checks, all 250 tests and exact replay of 42 statistical outputs pass; the [reproduction guide](docs/prkd2-reproduction.md) preserves the complete scope and missing-data requirements.
