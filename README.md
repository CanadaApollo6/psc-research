...
## Completed computational phase — September 16, 2026

**Five admitted questions are complete, independently checked and replayed within their declared scopes.** The [final synthesis](reports/psc-computational-frontier-summary.md) separates findings, biological leads and missing evidence. The current finite frontier is closed. Held sources and parked reserves require new qualifying evidence or a separately justified question; they are not an automatic backlog.

**Completed context analysis: MacroMap stimulus context.** Adding the fixed ETS2 gRNA1-down score lowers held-out nine-class log loss from **1.660703 to 1.529782**: gain **0.130921**, a **7.88% relative loss reduction**, with fixed-OOF conditional 95% interval **0.124191–0.136547**. This supports one fixed model comparison in healthy-derived macrophages. It does not establish unique ETS2 information, PSC specificity or causality. The stronger nonoverlap sensitivity does not replace the primary result.
...
**The fixed pediatric liver analysis is complete.** All 30 Core program comparisons have nominal P ≥ 0.382808, with no BH or BY discovery. The focal PSC−AIH ETS2 gRNA1-down difference is −0.0211 (95% CI −0.1547 to +0.1125). [Complete results, all 60 Core/Strict rows and verification](reports/psc-liver-programs.md) preserve the null result without claiming equivalence or absence of cell-specific biology.

**The single BACH2 RNA check is complete.** Among four SNP and four noSNP male PSC donors, the fixed C0/C1 relative-RNA difference is **−0.4441**, nominal 95% CI **[−1.0943, +0.2061]**, P **0.1433**. All eight omission points stay negative, but this does not establish significance, RNA equivalence or a protein/translation mechanism. [Full results and all fixed diagnostics](reports/psc-bach2-naive-rna.md), [figure](reports/figures/psc-bach2-naive-rna.png) and [reproduction](docs/psc-bach2-naive-rna-reproduction.md) complete the same-cohort check.

| Branch | Current decision |
|---|---|
| **PSC disease data** | Blood, organoid and [fixed pediatric liver](reports/psc-liver-programs.md) analyses complete. The liver panel has no detected group mean separation; keep 17 PSC, 17 ASC and 30 AIH separate. Whole-biopsy/clinical limits remain. Epithelial IL17 transfer is still HOLD, not fulfilled by unsigned ECM. |
...
| **BACH2 genotype** | [Complete one-endpoint RNA result](reports/psc-bach2-naive-rna.md), independently audited, scientifically replayed and fully reported. Eight male PSC donors, not 16,659 cell replicates. Negative but imprecise contrast; no equivalence, count-likelihood certificate, protein/translation claim or independent biological validation. |
...
The [completed agenda](docs/psc-analysis-agenda.md), [roadmap](ROADMAP.md) and [final reserve appraisal](reports/psc-remaining-frontier-appraisal.md) record the finite stopping boundary. GSE255234 heme/LPS and GSE303012 H69 are PARKED for limited incremental discrimination, not an invented access failure. Both admitted final endpoints are reported; no null result triggers a rescue sweep. This closes the present computational phase, not all possible PSC science. Earlier scientific results remain below. No researcher contact, service enrollment, paid compute or clinical recommendation occurred.
...
The [IBDverse original-model audit](reports/ibdverse-model-audit.md) recovers **94 classical and 93 intermediate blood-monocyte RNA-eligible donor labels**, TensorQTL 1.0.9 as the reported version, and one selected PRKD2 conditional hit per context. Expression-PC counts of 18/20 are consistent only if these are the executed model sample counts; neither is directly confirmed. The intermediate lead belongs to the study's rs313839 clump, but that membership supplies no signed study LD. Actual covariate/run records and adequate RNA/PSC covariance or complete RNA components remain required. Original source values, including a browser-versus-archive q-value discrepancy, are preserved in the [audit tables and reproduction guide](docs/ibdverse-model-audit-reproduction.md). No new shared-cause posterior is reported.

The [PRKD2 regulatory follow-up](reports/prkd2-regulatory-mechanism.md) finds **more coherent mechanistic support for rs313839 than for the newly recovered rs112445263** in the fixed model/ENCODE panel. PRKD2 ranks 1/50 versus 12/50; the new variant's small RNA prediction opposes the recovered association direction. Peak overlaps are 6/8 versus 0/8, but seven selected experiments share donors with published model training sources. Strong public-reference LD between the anchors leaves tagging plausible and causality unresolved. All 274 tests, independent numerical/peak checks and 17-file offline replay pass. The [overview](reports/figures/prkd2-mechanism/overview.png) and [reproduction guide](docs/prkd2-mechanism-reproduction.md) preserve the comparison shortfalls and training limits.

The [four-direction computational roadmap](docs/computational-followup-roadmap.md) records original UBASH3A RNA reads, the alternative-ending sequence, public peptide evidence and PRKD2 genetic-signal analysis. All four bounded computational stages are complete. These are tracked separately from the unperformed laboratory proposal.

The [PRKD2 public-data recovery](reports/prkd2-public-data-recovery.md) finds a useful new source: **original IBDverse classical and intermediate monocytes contain the missing rs112445263 association and clear the marginal coverage gate**. The intermediate-monocyte association has nominal P=1.153×10⁻⁹, with working risk C associated with lower RNA. The screen records all 42 Catalogue contexts and the bounded author-source follow-up, including unavailable files; the S100A8/9 context has a separate severe distribution limitation. Study-matched LD/component vectors and exact model/sample information remain required, so no new shared-cause posterior is reported. The [reproduction guide](docs/prkd2-recovery-reproduction.md) records the full outcomes and independent checks.

The preceding [PRKD2 signal analysis](reports/prkd2-signal-analysis.md) remains **inconclusive for a shared PSC–monocyte RNA cause**. Complete published RNA component vectors and a full-allele recovery still leave only about 80% of the PSC signal represented in those reprocessed cohorts; all 72 component/prior/sample-size settings fail the fixed coverage requirement. One missing variant, rs112445263, carries about 20% of PSC evidence and has no association rows in either reprocessed RNA file. Earlier expression-direction agreement remains intact. Independent checks, all 250 tests and exact replay of 42 statistical outputs pass; the [original reproduction guide](docs/prkd2-reproduction.md) preserves that completed scope.

The [public-protein analysis](reports/ubash3a-proteomics-analysis.md) searches a fixed fourteen-file naïve-CD4 experiment containing **66,766 MS2 spectra**. All four T29-ending peptides pass the retrieved-reference specificity check, but **none appears among 269,986 exported assignments**. Ordinary UBASH3A also has no assignment passing the global threshold, so this experiment does not strongly constrain the candidate's biological absence. Independent XML/PIN verification agrees; 24 small outputs and two large tables replay exactly, and all **238 tests pass**. The [reproduction guide](docs/ubash3a-proteomics-reproduction.md) preserves the sources, search settings and documented parser repairs.

The [original-read audit](reports/ubash3a-raw-read-audit.md) examines all **8,401,078 reads** in the TRAILS naïve and memory CD4 libraries. **Zero exact +29 alignments are found; 80 normal-junction reads pass the fixed primary criteria.** A second alignment setting agrees for nominated target reads. The catalog's alternative-ending RNA is not confirmed in these two libraries; other subsets, RNA survival and protein existence remain unresolved. Complete input totals, independent read classifications and deterministic replay agree; 212 tests passed at that stage.

The [alternative-ending reconstruction](reports/ubash3a-alternative-ending.md) is complete: the fixed TRAILS candidate gives a 4,392-nt reference RNA and, under a shared-start assumption, a 482-aa product with a stop in the final exon. Both hypothetical allele products are independently verified. Its four terminal peptides were carried into the subsequent public-protein analysis above. The [initial inventory](reports/ubash3a-proteomics-readiness.md) records all 273 PXD000376 files; the completed search uses one prospectively selected fourteen-file experiment. Neither a surviving RNA nor a protein is established by the conditional sequences.

A [primary-CD4 laboratory brief](docs/ubash3a-cd4-experiment-brief.md), [proposed analysis plan](docs/ubash3a-cd4-analysis-plan.md) and [portable handoff packet](reports/ubash3a-cd4-lab-handoff.zip) now specify the next experiment: identify the complete +29 RNAs, then measure their decay under a qualified NMD perturbation. **Preparation is complete; the experiment has not started.** Laboratory/sample access and assay qualification remain necessary. The packet includes 11 structure targets, reference sequences and empty measurement templates. Its proposed four-donor pilot is a feasibility allocation, not a powered confirmation study.

The preceding [UBASH3A public-data hypothesis test](reports/ubash3a-hypothesis-test.md) finds **one exact +29 catalog candidate, with a different RNA ending**, across 13 inspected annotation files. TRAILS assigns support in 10 of 11 CD4 subsets from one donor; the predicted boundary downstream arrangement is not found with either normal or +29 splicing. Eight original UPF1-experiment read extracts contain only one unspliced alignment overlapping the gene, leaving every target decay contrast **not estimable**. The [figure](reports/figures/ubash3a-hypothesis-test.png), full source/coordinate tables, independent checks and 12 diagnostic RNA windows are available. The specific survival hypothesis remains unresolved; catalog support is not proof of stable RNA or protein.

The preceding [UBASH3A transcript audit](reports/ubash3a-transcript-consequences.md) reconstructs **20 scenarios across five transcript models**. Conditional on exact +29-base retention, the canonical possible protein changes from **661 to 536 amino acids**, preserving a 465-aa prefix and gaining a 71-aa altered tail. Four models meet the pre-set positional RNA-decay flag; one is a 49/51-nt boundary case. UBA/SH3 reference sequences remain intact while the phosphatase-like region is interrupted. The [figure](reports/figures/ubash3a-transcript-consequences.png), FASTA files and complete maps are available; independent reconstruction agrees, all ten primary outputs replay exactly, and 165 repository tests passed at that stage. Splice-form abundance and protein survival remain experimental questions, so 3D modeling is deferred.

The preceding [GSE84161 supplement follow-up](reports/gse84161-supplement-followup.md) resolves the study inhibitors as **G-432 (TPL2)** and **G-573 (MEK)** from explicit original-supplement labels. Published structures are also located. Anonymous donor correspondence and the approximately threefold paper/GEO array-dose discrepancy remain unresolved; the separate table archive could not be retrieved. The 30 source sample labels and remaining questions are recorded without assigning donors or calculating new effects.

The completed [GSE84161 normalization and mapping](reports/gse84161-normalization-and-mapping.md) processes all **30 original arrays**, preserving **54,675 probe sets** and aggregating **20,486 Entrez genes**. The primary ETS2 program maps **801/927 genes (86.4%)**, with 652 remaining after comparator-overlap removal. Eight of nine programs pass mapping gates; the gRNA1-up companion fails at 77.5%. Independent checks match every gene median exactly, and all 144 tests passed at that stage. No program scores or treatment contrasts have been calculated. The earlier [study investigation](reports/gse84161-study-investigation.md) and unsent [source questions](docs/gse84161-source-questions.md) remain preserved; the supplement follow-up supersedes their compound-code uncertainty.

The preceding [independent-validation preparation](reports/ets2-validation-readiness.md) identifies primary-human MEK-pharmacology and LPS/heme context studies, preserving their assay and metadata gaps. GSE255234's 16 samples and 12 planned donor pairs are pinned. An amended prospective protocol uses control-only expression matching and keeps direct ETS2 perturbation, MEK response, generic stimulation and source reuse distinct. No fully eligible independent primary-human direct-ETS2 experiment was found in this finite search. A [focused original ETS2 author email](docs/ets2-lee-request-draft.md) and [reproducibility brief](docs/ets2-lee-reproducibility-brief.md) are drafted and unsent.

The preceding [ETS2 program benchmark](reports/ets2-program-benchmark.md) measures a published macrophage gene set in PSC liver cells, with expression-matched references and broad inflammation/stress comparisons. **906 of 927 intended genes map; the primary activated-macrophage score is lower than Kupffer cells in all four eligible donor pairs**, including after removing direct comparator-gene overlap. Related disruption experiments agree, but overexpression reverses the program only partially. This is useful evidence about transfer between cell states, not a validated disease-activity score. A separate [inhibitor-method reconstruction](docs/ets2-inhibitor-reconstruction.md) recomputes all nine raw pathway enrichment scores from the released 500-nM ranks and compares archived NES with publisher data; the historical null, original count fit and rank-export provenance remain incomplete.

The preceding [liver-cell context analysis](reports/liver-atlas-cell-context.md) checks **195,417 public cells and nuclei**, retaining 181,125 under fixed annotation rules. Donor-level measurements support **UBASH3A in CD4 T cells and ETS2/PRKD2 in monocytes** as contexts for the existing hypotheses. All five candidates, including PFKFB3 and BCL2L11, have usable measurements in the selected PSC populations. Whole-cell controls use a different chemistry, and the relevant nuclear immune-cell groups have too few eligible controls for our disease comparison. This establishes measurement context, not a PSC-specific change or treatment effect.
...
- A frozen nine-program ETS2 follow-up with 144 context summaries, 108 paired contrasts, 80 descriptive correlations, publisher-table source audits and a reproducible inhibitor ranking-to-pathway reconstruction. See the [reproduction guide](docs/ets2-program-reproduction.md).
- Separate perturbation and stimulation-context dataset inventories, preserved original/amended validation rules, a four-donor sample/contrast preparation manifest, independent count-file structural checks, and an unsent original-study provenance request. See the [readiness decision](reports/ets2-validation-readiness.md).
- A reproducible GSE84161 sample/annotation audit, checksum and header validation of all 30 original arrays, publication/compound evidence review, and a prospective array framework with 35 candidate comparisons across seven contrasts. See the [study investigation and reproduction commands](reports/gse84161-study-investigation.md).
- Completed GSE84161 all-array RMA, full probe/Entrez matrices, deterministic mapping and all nine coverage outcomes, raw/RMA/RLE diagnostics, a pinned runtime with a documented source-build repair, and independent numerical verification. See the [normalization reproduction guide](docs/gse84161-reproduction.md).
