# Frozen PRKD2 comparison using the existing pinned coloc/susieR runtime.
source('scripts/run_coloc_platform.R')

prkd2_read <- function(id, suffix='PRKD2') {
  read.csv(gzfile(file.path('data/derived/prkd2-inputs',paste0(id,'-',suffix,'.csv.gz'))),
...
prkd2_baseline <- function(plan) {
  ...
  write.csv(data.table::rbindlist(rows),'reports/prkd2-baseline.csv',row.names=FALSE)
  con <- gzfile('reports/prkd2-baseline-variant-posteriors.csv.gz','wt')
  write.csv(data.table::rbindlist(detail),con,row.names=FALSE);close(con)
  write_json(notes,'reports/prkd2-baseline-warnings.json')
}

prkd2_fit <- function(plan) {
  trait <- prkd2_read('GWAS','PRKD2-ld'); m <- nrow(trait)
  prefix <- 'data/raw/prkd2-ld/GWAS-PRKD2'
  ...
  for(n in c(11386,3504,14890)) {
    ...
    write.csv(diagnostic$conditional_dist,paste0('reports/prkd2-LD-diagnostics-N',n,'.csv'),row.names=FALSE)
    ...
    if(is.null(fit$error)) {
      ...
      write.csv(data.frame(snp=trait$snp,pip=fit$pip),paste0('reports/prkd2-GWAS-pips-N',n,'.csv'),row.names=FALSE)
      con <- gzfile(paste0('data/derived/prkd2-GWAS-component-bfs-N',n,'.csv.gz'),'wt')
      write.csv(data.frame(snp=trait$snp,t(fit$lbf_variable),check.names=FALSE),con,row.names=FALSE);close(con)
    }
    write_json(record,paste0('reports/prkd2-GWAS-fit-N',n,'.json'))
...
prkd2_published <- function(ds, indices) {
  rows <- read.delim(gzfile(paste0('data/derived/prkd2-query-rows/',ds,'-PRKD2-published-bfs.tsv.gz')),
                     stringsAsFactors=FALSE,check.names=FALSE)
  if(!all(rows$molecular_trait_id=='ENSG00000105287'))stop('Wrong published phenotype')
  fields <- strsplit(rows$variant,'_',fixed=TRUE)
  keys <- vapply(fields,function(f){
    if(length(f)!=4)stop('Malformed published variant')
  ...
  if(anyDuplicated(keys))stop('Duplicate full published BF identity')
  mat <- t(as.matrix(rows[,paste0('lbf_variable',indices),drop=FALSE]))
  if(any(is.na(mat)|mat==Inf))stop('Invalid component BF')
  colnames(mat) <- keys;rownames(mat) <- as.character(indices)
  ...
  for(ds in vapply(plan$datasets,function(d)d$dataset_id,character(1))) {
    ...
    for(n in c(11386,3504,14890)) {
      ...
      for(i in fit$sets$cs_index)for(j in seq_along(ids)) {
        ...
        for(prior in priors) {
          captured <- character()
          result <- withCallingHandlers(coloc::coloc.bf_bf(gb,mb,p1=plan$methods$priors$p1,p2=plan$methods$priors$p2,
