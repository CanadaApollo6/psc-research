...
Every transferred byte range is cached and hashed. Row-group statistics select
the gene, followed by exact row filtering. Metadata mode does not read effects.
"""
...
def main():
    ...
    access_path = ROOT / 'config/coverage-repair-alternative-parquet-plan.json'
    ...
    manifest = json.loads(path.read_text()) if path.exists() else {**pins, 'queries': [dict(s, ranges=[]) for s in access['sources']]}
    ...
    for spec in manifest['queries']:
        ...
        gene_index = schema.index(gene_column)
        selected = []
        for i in range(parquet.metadata.num_row_groups):
            statistics = parquet.metadata.row_group(i).column(gene_index).statistics
            if not statistics or not statistics.has_min_max: raise ValueError('Missing row-group gene bounds; do not scan whole archive')
            lo, hi = statistics.min, statistics.max
            if isinstance(lo, bytes): lo, hi = lo.decode(), hi.decode()
            if lo <= plan['gene_id'] + '.999999' and hi >= plan['gene_id']:
                selected.append(i)
        ...
        frame = frame[frame[gene_column].str.split('.').str[0].eq(plan['gene_id'])]
