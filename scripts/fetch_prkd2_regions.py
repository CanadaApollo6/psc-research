...
def main():
    ...
    plan = verify_plan()
    sources_path = ROOT / 'config/prkd2-sources.json'
    ...
    path = ROOT / ('config/prkd2-' + args.stage + '-queries.json')
    frozen = {'plan_sha256': sha((ROOT/'config/prkd2-plan.json').read_bytes()),
    ...
    output = ROOT/'data/derived/prkd2-query-rows'
    ...
    def gwas_fetch(start, end):
        ...
        filename = f'prkd2-gwas-bytes-{start}-{end}.txt'
    ...
    if args.stage == 'gwas':
        ...
        for region in plan['regions']:
            ...
            retain(identifier, plain, 'data/raw/prkd2-'+identifier+'-dosages.tsv.gz',
            ...
            time.sleep(bounds['minimum_seconds_between_indexed_queries'])
    expected = len(plan['datasets'])*len(plan['regions']) if args.stage == 'qtl' else len(plan['regions'])
