...
def main():
    ...
    verify_plan()
    path = ROOT / ('config/prkd2-expanded-' + args.stage + '-execution.json')
    if path.exists():
    ...
    inputs = [ROOT / n for n in ['config/prkd2-plan.json', 'config/prkd2-sources.json',
        'reports/prkd2-expanded-input-audit.json', 'scripts/run_prkd2_expanded.R', 'scripts/execute_prkd2_expanded.py',
        'scripts/run_coloc.R', 'scripts/run_coloc_platform.R', 'config/prkd2-expanded-plan.json', 'config/prkd2-expanded-sources.json', 'reports/prkd2-normalization-verification.json']]
    inputs += sorted((ROOT / 'data/derived/prkd2-expanded-inputs').glob('*.csv.gz'))
    inputs += sorted((ROOT / 'data/raw/prkd2-expanded-ld').glob('*.f64'))
    if args.stage == 'compare':
        for ds in ['QTD000021', 'QTD000504']:
            ...
            inputs += [ROOT / ('data/derived/prkd2-expanded-published-bfs/' + ds + '.csv.gz'), ROOT / ('config/prkd2-expanded-bf-mapping-' + ds + '.json')]
        inputs += [ROOT / 'data/derived/prkd2-published-credible-sets.csv.gz']
        inputs += sorted((ROOT / 'data/raw/prkd2-expanded-ld').glob('*.rds'))
        inputs += sorted((ROOT / 'reports').glob('prkd2-expanded-GWAS-fit-*.json'))
    before = {p: sha(p.read_bytes()) for folder in ['reports', 'data/derived', 'data/raw/prkd2-expanded-ld']
              for p in (ROOT / folder).glob('prkd2-expanded-*') if p.is_file()}
    before.update({p: sha(p.read_bytes()) for p in (ROOT / 'data/raw/prkd2-expanded-ld').glob('*.rds')})
    ...
    log = ROOT / ('work/prkd2/expanded-' + args.stage + '.log')
    command = [str(ROOT / 'work/coloc-runtime/r/bin/Rscript'), '--vanilla', 'scripts/run_prkd2_expanded.R', args.stage]
    ...
    for folder in ['reports', 'data/derived']:
        for p in (ROOT / folder).glob('prkd2-expanded-*'):
            if p.is_file() and (p not in before or sha(p.read_bytes()) != before[p]):
                ...
                changed.append(p)
    changed += [p for p in (ROOT / 'data/raw/prkd2-expanded-ld').glob('*.rds') if p not in before or sha(p.read_bytes()) != before[p]]
